...
const ANALYTICS_DATA_FILE = process.env.ANALYTICS_DATA_FILE || path.join(os.tmpdir(), 'vrlab-analytics.json');
...
const httpServer = http.createServer((req, res) => {
  ...
  if (pathname === '/api/analytics' && req.method === 'GET') {
    jsonResponse(res, buildAnalyticsSnapshot());
    return;
  }
...
function handleAgentConnection(ws) {
  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'session_start') {
      if (msg.event && msg.event !== 'session_start') {
        const open = findOpenSession(deviceId, msg.sessionId);
        if (open) {
          broadcastToDashboard({ type: 'session_update', session: open });
          return;
        }
      }

      const session = {
        sessionId:   msg.sessionId || `${deviceId}-${msg.ts || Date.now()}`,
        deviceId,
        ...
        studentName: msg.studentName || msg.userName || 'Unknown',
        studentId:   msg.studentId || '',
        isGuest:     !!msg.isGuest,
        event:       msg.event || 'session_start',
        startTime:   Date.now(),
        endTime:     null,
        durationSeconds: 0,
      };
      sessions.set(session.sessionId, session);
      deviceUsers.set(deviceId, session.userName);

      // Update device info with current user
      if (devices.has(deviceId)) devices.get(deviceId).info.currentUser = session.userName;

      saveAnalytics();
      broadcastToDashboard({ type: 'session_start', session });
      console.log(`[Session] ${session.userName} (${session.studentId || 'no-id'}) started on ${session.deviceName}`);
      return;
    }

    if (msg.type === 'session_end') {
      let found = findOpenSession(deviceId, msg.sessionId);
      if (found) {
        found.endTime        = Date.now();
        found.durationSeconds = msg.durationSeconds || Math.round((found.endTime - found.startTime) / 1000);
        found.endReason = msg.reason || '';
        found.studentName = msg.studentName || found.studentName;
        found.studentId = msg.studentId || found.studentId;
        found.isGuest = msg.isGuest ?? found.isGuest;
        saveAnalytics();
    ...
    if (msg.type === 'training_start') {
      const event = {
        eventId: `${msg.sessionId || deviceId}-${msg.trainingPackage || 'training'}-${msg.ts || Date.now()}`,
        type: 'training_start',
        sessionId: msg.sessionId || '',
        deviceId,
        userName: msg.userName || deviceUsers.get(deviceId) || 'Unknown',
        studentName: msg.studentName || msg.userName || 'Unknown',
        studentId: msg.studentId || '',
        isGuest: !!msg.isGuest,
        trainingName: msg.trainingName || msg.trainingPackage || 'Unknown Training',
        trainingPackage: msg.trainingPackage || '',
      ...
      saveAnalytics();
      broadcastToDashboard({ type: 'training_start', training: event });
      console.log(`[Training] ${event.studentName} started ${event.trainingName}`);
      return;
    }

    if (msg.type === 'training_end') {
      const event = findOpenTrainingEvent(deviceId, msg.sessionId, msg.trainingPackage);
      const ended = event || {
        eventId: `${msg.sessionId || deviceId}-${msg.trainingPackage || 'training'}-${msg.ts || Date.now()}`,
        type: 'training_end',
        sessionId: msg.sessionId || '',
      ...
      ended.type = 'training_end';
      ended.endedAt = msg.endedAt || Date.now();
      ended.durationSeconds = msg.durationSeconds || Math.max(0, Math.round((ended.endedAt - ended.startedAt) / 1000));
      ended.result = msg.result || 'ended';
      if (!event) trainingEvents.push(ended);
      saveAnalytics();
      broadcastToDashboard({ type: 'training_end', training: ended });
      console.log(`[Training] ${ended.studentName} ended ${ended.trainingName} (${ended.durationSeconds}s)`);
      return;
    }
...
function loadAnalytics() {
  try {
    if (!fs.existsSync(ANALYTICS_DATA_FILE)) return;
    const raw = fs.readFileSync(ANALYTICS_DATA_FILE, 'utf8');
    const data = JSON.parse(raw);

    if (Array.isArray(data.sessions)) {
      for (const session of data.sessions) {
        if (session && session.sessionId) sessions.set(session.sessionId, session);
      }
    }

    if (Array.isArray(data.trainingEvents)) {
      trainingEvents.push(...data.trainingEvents.filter(Boolean));
    }

    console.log(`[Analytics] Loaded ${sessions.size} sessions and ${trainingEvents.length} training events`);
  } catch (err) {
    console.warn(`[Analytics] Failed to load ${ANALYTICS_DATA_FILE}: ${err.message}`);
  }
}
...
function saveAnalytics() {
  try {
    ...
    console.warn(`[Analytics] Failed to save ${ANALYTICS_DATA_FILE}: ${err.message}`);
  }
}

function findOpenSession(deviceId, sessionId) {
  ...
  let found = null;
  for (const s of sessions.values()) {
    if (s.deviceId === deviceId && !s.endTime) found = s;
  }
  return found;
}

function findOpenTrainingEvent(deviceId, sessionId, trainingPackage) {
  for (let i = trainingEvents.length - 1; i >= 0; i--) {
    const event = trainingEvents[i];
    if (!event || event.endedAt) continue;
    if (event.deviceId !== deviceId) continue;
    if (sessionId && event.sessionId !== sessionId) continue;
    if (trainingPackage && event.trainingPackage !== trainingPackage) continue;
    return event;
  }
  return null;
}
...
function buildAnalyticsSnapshot() {
  const sessionList = [...sessions.values()].sort((a, b) => (b.startTime || 0) - (a.startTime || 0));
  const trainingList = [...trainingEvents].sort((a, b) => (b.startedAt || 0) - (a.startedAt || 0));
  const studentMap = new Map();
  const sessionsByDay = new Map();
  const trainingMap = new Map();

  for (const s of sessionList) {
    const key = s.isGuest ? `guest:${s.sessionId}` : (s.studentId || s.userName || 'Unknown');
    if (!studentMap.has(key)) {
      studentMap.set(key, {
        studentId: s.studentId || (s.isGuest ? 'guest' : ''),
        studentName: s.studentName || s.userName || 'Unknown',
        userName: s.userName || s.studentName || 'Unknown',
        isGuest: !!s.isGuest,
        totalSeconds: 0,
        sessionCount: 0,
        trainingSeconds: 0,
    ...
    student.totalSeconds += s.durationSeconds || 0;
    student.sessionCount += 1;
    if ((s.startTime || 0) > student.lastSeen) {
      student.lastSeen = s.startTime || 0;
      student.lastDevice = s.deviceName || '';
      student.lastHeadset = s.headsetModel || '';
    }

    const day = new Date(s.startTime || Date.now()).toISOString().slice(0, 10);
    sessionsByDay.set(day, (sessionsByDay.get(day) || 0) + 1);
  }

  for (const t of trainingList) {
    const key = t.isGuest ? `guest:${t.sessionId || t.eventId}` : (t.studentId || t.userName || 'Unknown');
    if (!studentMap.has(key)) {
      studentMap.set(key, {
        studentId: t.studentId || (t.isGuest ? 'guest' : ''),
        studentName: t.studentName || t.userName || 'Unknown',
        userName: t.userName || t.studentName || 'Unknown',
        isGuest: !!t.isGuest,
        totalSeconds: 0,
        sessionCount: 0,
        trainingSeconds: 0,
    ...
    const student = studentMap.get(key);
    student.trainingSeconds += t.durationSeconds || 0;
    student.trainingCount += t.endedAt ? 1 : 0;

    const trainingName = t.trainingName || t.trainingPackage || 'Unknown Training';
    if (!trainingMap.has(trainingName)) {
      trainingMap.set(trainingName, {
        trainingName,
        trainingPackage: t.trainingPackage || '',
        starts: 0,
        completions: 0,
        totalSeconds: 0,
      });
    }
    const training = trainingMap.get(trainingName);
    training.starts += t.startedAt ? 1 : 0;
    if (t.endedAt) training.completions += 1;
    training.totalSeconds += t.durationSeconds || 0;
  }

  const totalSessionSeconds = sessionList.reduce((sum, s) => sum + (s.durationSeconds || 0), 0);
  const totalTrainingSeconds = trainingList.reduce((sum, t) => sum + (t.durationSeconds || 0), 0);

  return {
    summary: {
      totalStudents: [...studentMap.values()].filter(s => !s.isGuest).length,
      totalUsers: studentMap.size,
      activeSessions: sessionList.filter(s => !s.endTime).length,
      totalSessions: sessionList.length,
      completedSessions: sessionList.filter(s => s.endTime).length,
      guestSessions: sessionList.filter(s => s.isGuest).length,
      totalSessionSeconds,
      avgSessionSeconds: sessionList.length ? Math.round(totalSessionSeconds / sessionList.length) : 0,
      totalTrainingSeconds,
      trainingStarts: trainingList.length,
      trainingCompletions: trainingList.filter(t => t.endedAt).length,
    },
    students: [...studentMap.values()].sort((a, b) => b.totalSeconds - a.totalSeconds),
    sessions: sessionList,
    trainingEvents: trainingList,
    charts: {
      sessionsByDay: [...sessionsByDay.entries()].sort(([a], [b]) => a.localeCompare(b))
        .map(([date, count]) => ({ date, count })),
      trainingUsage: [...trainingMap.values()].sort((a, b) => b.totalSeconds - a.totalSeconds),
      topStudentsByTime: [...studentMap.values()].sort((a, b) => b.totalSeconds - a.totalSeconds).slice(0, 8),
    },
  };
}
