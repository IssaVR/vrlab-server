  ...
  ws.on('message', (raw) => {
    ...
    if (msg.type === 'command_relay') {
      ...
      if (target && target.ws.readyState === WebSocket.OPEN) {
        ...
        console.log(`[CMD Relay] SUCCESS → ${msg.deviceId} : ${msg.command}`);
        ws.send(JSON.stringify({ type: 'command_ack', ok: true,  deviceId: msg.deviceId, command: msg.command }));
      } else {
        const reason = !target ? 'device not in registry' : `ws state=${target.ws.readyState}`;
        console.log(`[CMD Relay] FAILED: ${reason}`);
        ws.send(JSON.stringify({ type: 'command_ack', ok: false, error: `Device not connected (${reason})` }));
      }
    }
  });
